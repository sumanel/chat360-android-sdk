...
class ChatComposeFragment : Fragment() {
    ...
    private val viewModel: ChatViewModel by viewModels {
        ChatViewModel.Factory(context = requireContext().applicationContext, baseUrl = config.baseUrl, botId = config.botId, historyEnabled = config.historyEnabled, clientExternalName = config.clientExternalName, agentCode = config.agentCode, suppressInitialBotMessages = config.Chat360UIConfig.behavior.suppressInitialBotMessages)
    }
    ...
    companion object {
        ...
        fun newInstance(
            ...
            arguments = Bundle().apply {
                ...
                putString(EXTRA_CLIENT_EXTERNAL_NAME, clientExternalName)
                putString(EXTRA_AGENT_CODE, agentCode)
            }
        }
    }
}
